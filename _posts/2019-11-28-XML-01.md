...
```
schema标签约束
* <sequence>约束标签表示包裹的标签元素必须顺序出现
* <all>约束标签表示其包裹的标签元素只能出现一次
* <choice>约束标签表示包裹的元素中只能出现其中一个
* <any>约束标签表示包裹的元素可以是任意的
* maxOccurs="unbounded"属性标签表示含有该属性的标签可以出现任意次  

schema属性约束必须写在复杂元素里面，且写在</complextType>标签之前，写法为<attribute name="id1" type="int" use="required"></attribute>
